...
const paths = {
  ...
  srcjs: ['dist/src/**/*.js'],
  testts: ['tests/**/*.ts'],
  testjs: ['tests/unit/**/*.test.js', 'dist/tests/unit/**/*.test.js']
}
...
//* BUILD
//* *****************************************************************************
let tsLibProject = tsc.createProject('tsconfig.json', { target: 'es2015', typescript: require('typescript') })
...
let tsEsProject = tsc.createProject('tsconfig.json', { target: 'es2016', typescript: require('typescript') })
...
//* DEFAULT
//* *****************************************************************************
gulp.task('build', gulp.series(gulp.parallel('clean-tsc', 'lint'), gulp.parallel('build-src', 'build-test', 'build-lib')))
...
const watch = function () {
  wrap(gulp.watch(paths.ts.concat(paths.testjs), gulp.series('test')), 'watch')
}
