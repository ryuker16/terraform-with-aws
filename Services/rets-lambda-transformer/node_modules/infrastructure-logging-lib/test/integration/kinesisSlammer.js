...
var kinesis = Promise.promisifyAll(require('kinesis'))
...
function setExports () {
  exports.slamKinesis = slamKinesis
...
function main (args) {
  ...
  if (delay !== 0) {
    ...
    setInterval(slamKinesis, delay, count)
  }
}

function slamKinesis (num) {
  var stream = createKinesisLogStream(config)
...
/**
 * Creates a stream that recieves JSON object messages
 * and transforms and sends them to Kinesis
 */
function createKinesisLogStream (cfg) {
  var kinesify = new streams.KinesifyTransform()
  var streamConnection = Object.assign({name: cfg.stream.name}, cfg.connection)
  var kinesisStream = kinesis.stream(streamConnection)
  kinesify.pipe(kinesisStream)
  return kinesify
}
